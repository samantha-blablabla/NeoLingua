...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col max-w-md mx-auto relative bg-[#0A0A0A] shadow-2xl border-x border-zinc-900">
      <GrainOverlay />
      ...
      <header className="p-6 pt-10 flex justify-between items-end">
        ...
        <button className="w-12 h-12 rounded-2xl bg-zinc-900 flex items-center justify-center border border-zinc-800 hover:border-[#CCFF00] transition-colors">
      ...
      <main className="flex-1 overflow-y-auto px-6 pb-24 space-y-8">
        ...
        <div className="flex gap-4">
          <div className="flex-1 bg-zinc-900/50 p-4 rounded-[24px] border border-zinc-800">
        ...
        <section>
          ...
          <div className="relative group overflow-hidden bg-zinc-900 rounded-[32px] p-8 border border-zinc-800 shadow-xl transition-all hover:border-[#CCFF00]/50 min-h-[400px] flex flex-col justify-between">
            {/* Grain Texture (Local) */}
            <div className="absolute inset-0 opacity-[0.07] pointer-events-none mix-blend-overlay bg-white"></div>
            
            {loading ? (
              <div className="flex flex-col items-center justify-center h-full space-y-4 py-20">
                <div className="w-12 h-12 border-4 border-[#CCFF00]/20 border-t-[#CCFF00] rounded-full animate-spin"></div>
                <p className="text-zinc-500 font-heading">AI đang soạn giáo án...</p>
              </div>
            ) : error ? (
              <div className="py-20 text-center">
                <p className="text-red-400 mb-4">{error}</p>
                <button onClick={fetchNewLesson} className="px-6 py-2 bg-[#CCFF00] text-black font-bold rounded-full">Thử lại</button>
              </div>
            ) : lesson ? (
              <>
                <div className="space-y-6">
                  <div>
                    <span className="inline-block px-3 py-1 bg-[#CCFF00] text-black text-[10px] font-black rounded-full mb-3 uppercase tracking-tighter">
                      WORD OF THE DAY
                    </span>
                ...
                <div className="mt-8">
                  <button className="w-full py-5 bg-[#CCFF00] text-black font-heading font-black text-lg rounded-[20px] shadow-[0_8px_0_0_#AACC00] active:shadow-none active:translate-y-1 transition-all">
                    START PRACTICE
                  </button>
                </div>
              </>
        ...
        <section className="grid grid-cols-2 gap-4">
          <div className="p-6 bg-[#BFA3FF] rounded-[28px] text-black">
          ...
          <div className="p-6 bg-[#FF6B4A] rounded-[28px] text-black">
      ...
      <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-black/80 backdrop-blur-xl border-t border-zinc-800 px-8 py-4 flex justify-between items-center z-[50]">
        ...
        <div className="w-14 h-14 bg-[#CCFF00] rounded-full flex items-center justify-center -translate-y-8 shadow-xl shadow-[#CCFF00]/20 border-[6px] border-[#0A0A0A]">
